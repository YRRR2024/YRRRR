...
@plugins.register(
    ...
    version="1.0.0",  # 可以直接在这里定义版本
    author="YRRR",    # 可以直接在这里定义作者
)
class RenderHtmlAsImage(Plugin):
    def __init__(self):
        super().__init__()
        self.handlers[Event.ON_DECORATE_REPLY] = self.on_decorate_reply
        logger.info("[RenderHtmlAsImage] initialized.")

    def on_decorate_reply(self, e_context: EventContext):
        html_code = e_context["reply"].content.strip()  # 假设回复内容是 HTML 代码
        image_path = self.render_html_to_image(html_code)  # 渲染 HTML 为图片

        if image_path:
            image_url = self.upload_image(image_path)  # 调用内部方法上传图片
            if image_url:
    ...
    def upload_image(self, image_path):
        ...
        try:
            ...
            logger.error(f"[RenderHtmlAsImage] Error uploading image: {e}")
            return None
